...
class CoustomPaginatedDataTable extends StatefulWidget {
  ...
  /// particular [CoustomPaginatedDataTable] widget is created; avoid creating a new
  /// [DataTableSource] with each new instance of the [CoustomPaginatedDataTable]
  ...
  /// To modify the border or background color of the [CoustomPaginatedDataTable], use
  /// [CardTheme], since a [Card] wraps the inner [DataTable].
  CoustomPaginatedDataTable({
    super.key,
    this.header,
    this.actions,
  ...
  /// [CoustomPaginatedDataTable] widget itself; it should be reused each time the
  /// [CoustomPaginatedDataTable] constructor is called.
  ...
  @override
  CoustomPaginatedDataTableState createState() =>
      CoustomPaginatedDataTableState();
}

/// Holds the state of a [CoustomPaginatedDataTable].
///
/// The table can be programmatically paged using the [pageTo] method.
class CoustomPaginatedDataTableState extends State<CoustomPaginatedDataTable> {
  ...
  void getpagelist(int totalpages) {
    List innerlist = [];
    for (int i = 0; i < totalpages; i++) {
      if (i % 5 == 0) {
        innerlist.addAll([i, "..."]);
        pageList.add(innerlist);
        innerlist.clear();
      }
      innerlist.add(i);
    }
  ...
  @override
  void didUpdateWidget(CoustomPaginatedDataTable oldWidget) {
  ...
  void _handlePrevious() {
    ...
    controller1.animateTo(
      (math.max(_firstRowIndex - widget.rowsPerPage, 0)) * 5,
  ...
  void _handleNext() {
    ...
    if (page <
        ((_rowCount - 1) / widget.rowsPerPage).floor() * widget.rowsPerPage) {
      page++;
    }
    controller1.animateTo(
      (_firstRowIndex + widget.rowsPerPage) * 5,
      duration: const Duration(seconds: 1),
      curve: Curves.linear,
    );
    if (count % 3 == 0) {}
  ...
  void _handleLast() {
    ...
    page = ((_rowCount - 1) / widget.rowsPerPage).floor() * widget.rowsPerPage;
  }
                ...
                children: <Widget>[
                  ...
                  SingleChildScrollView(
                    ...
                    child: ConstrainedBox(
                      ...
                      child: DataTable(
                        dataTextStyle: GoogleFonts.poppins().copyWith(
                            color: const Color(0xff4A4A68), fontSize: 18),
                        headingTextStyle: TextStyle(
                            color: const Color(0xff2F3336).withOpacity(0.5),
                            fontSize: 15),
                        headingRowColor:
  ...
  ListView paginationList(int totalpage, int currentpage, int indicatorindex) {
    return ListView.builder(
      padding: const EdgeInsets.symmetric(horizontal: 4),
      physics: const NeverScrollableScrollPhysics(),
      scrollDirection: Axis.horizontal,
      controller: controller1,
      itemCount: (_rowCount / widget.rowsPerPage % 2 == 0)
          ? (_rowCount / widget.rowsPerPage).round()
          : (_rowCount / widget.rowsPerPage).floor() + 1,
      itemBuilder: (_, i) {
        return Container(
          width: 25,
          decoration: BoxDecoration(
            border: i + 1 == currentpage
                ? Border.all(color: Color(0xffDCDDDF), width: 1.0)
                : Border.all(color: Colors.transparent),
            borderRadius: BorderRadius.circular(3),
            shape: BoxShape.rectangle,
            color: i + 1 == currentpage ? Color(0xffDCDDDF) : null,
          ),
          child: Center(
              child: Text(
            (i + 1).toString(),
